    ...
    const init = async () => {
        if (document.querySelector('#fimg0')) {
            const div = Object.assign(document.createElement('div'), {
                ...
                innerHTML: '<h2 style="color:#FFF;margin:0 0 10px">Results:</h2><button style="position:absolute;right:10px;top:10px;background:none;border:none;color:#FFF;cursor:pointer" onclick="this.parentNode.style.display=\'none\'">X</button>'
            });
            document.body.appendChild(div);
            const urls = extractUrls(Math.min(Math.max(parseInt(prompt('How many URLs to extract? (1-50)', '10')) || 10, 1), 50));
            div.innerHTML += urls.length ? urls.map((item, i) => `<a href="${item.url}" target="_blank" style="color:#00FFFF;text-decoration:none;display:block;margin-bottom:10px">${i+1}. ${item.domain}</a>`).join('') : '<p>No URLs found</p>';
        }
    };
